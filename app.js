...
app.post("/register/", async (request, response) => {
  ...
  if (dbUser !== undefined) {
    ...
    if (password.length < 6) {
      ...
      const insertQuery = `INSERT INTO user(name,username,password,gender) VALUES('${name}','${username}','${hashPassword}','${gender}')`;
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  const getUserQuery = `SELECT tweet.tweet,COUNT(like.user_id)as likes,COUNT(reply.user_id) as replies,tweet.date_time as dateTime
                          FROM tweet
  ...
  const details = await database.all(getUserQuery);
